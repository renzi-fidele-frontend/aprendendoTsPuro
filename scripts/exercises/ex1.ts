...
async function apanharDados() {
   ...
   let aguardando = 0;
   let cancelado = 0;
   let melhorDia: "Segunda-feira" | "Terça-feira" | "Quarta-feira" | "Quinta-feira" | "Sexta-feira" | "Sábado" | "Domingo";
      ...
      .then((v: any[]) => {
         if (corpo) {
            ...
            arrayNormalizado.forEach(({ nome, email, valor, formaDePagamento, status, data }) => {
               ...
               if (formaDePagamento === "Cartão de Crédito") credito += 1;
               if (formaDePagamento === "Boleto") boleto += 1;

               // Calculando as estatísticas de vendas
               if (status === "Paga") pago += 1;
               if (status === "Recusada pela operadora de cartão") recusado += 1;
               if (status === "Aguardando pagamento") aguardando += 1;
               if (status === "Estornada") cancelado += 1;

               // Calculando o dia da semana com mais vendas
               let dataFormatada = new Date(data);
               let diaDaSemana = dataFormatada.getDay();
               if (diaDaSemana === 0) domingo += 1;
               else if (diaDaSemana === 1) segunda += 1;
               else if (diaDaSemana === 2) terca += 1;
               else if (diaDaSemana === 3) quarta += 1;
               else if (diaDaSemana === 4) quinta += 1;
               else if (diaDaSemana === 5) sexta += 1;
               else if (diaDaSemana === 6) sabado += 1;
               ...
               if (index === 0) melhorDia = "Segunda-feira";
               else if (index === 1) melhorDia = "Terça-feira";
               else if (index === 2) melhorDia = "Quarta-feira";
               else if (index === 3) melhorDia = "Quinta-feira";
               else if (index === 4) melhorDia = "Sexta-feira";
               else if (index === 5) melhorDia = "Sábado";
               else if (index === 6) melhorDia = "Domingo";
